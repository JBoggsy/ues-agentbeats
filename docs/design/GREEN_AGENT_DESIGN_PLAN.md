...
self._proctor_api_key: str                # Admin key for UES
...
The GreenAgent needs to start a UES server as a subprocess and capture the admin API key from stdout.

**Challenge**: UES prints the admin key to stdout during startup. We need to capture this.

**Proposed Approach**:

```python
async def _start_ues_server(self) -> tuple[subprocess.Popen, str]:
    """Start UES server and capture admin key.
    
    Returns:
        Tuple of (process, admin_api_key).
    """
    # Start UES as subprocess with pipe for stdout
    ...
    # Read output looking for admin key line
    admin_key = None
    for line in process.stdout:
        # UES prints: "   Secret: ues_xxxx..."
        if "Secret:" in line:
            admin_key = line.split("Secret:")[1].strip()
            break
    
    if not admin_key:
        process.terminate()
        raise RuntimeError("Failed to capture UES admin key")
    
    return process, admin_key
```

**Question**: Should we continue reading stdout for logging, or close it after getting the key?

**Decision**: Continue reading from stdout for logging. UES server output should be logged and stored attached to the Green agent.

**Note**: While UES output format could change in the future, parsing stdout is acceptable for now. A CLI flag or file output option for programmatic key retrieval may be added to UES later.
...
│                                                                   │
│  ┌─────────────────────┐                                         │
│  │   Admin Key         │   Created at UES server startup         │
│  │   (proctor-level)   │   Captured by GreenAgent                │
...
├── assessment/
│   ├── __init__.py                  # (NEW)
│   └── models.py                    # TurnResult dataclass (NEW)
└── ... (existing modules)
```
...
|------|-------------|--------------|-----------|
| 1 | Create `assessment/models.py` with `TurnResult` | None | 0.5h |
| 2 | Implement UES server management (`_start_ues_server`, `_wait_for_ues_ready`) | UES CLI | 2h |
